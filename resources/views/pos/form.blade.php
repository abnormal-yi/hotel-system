<x-layouts.app title="New POS Order">
    <div class="max-w-3xl mx-auto space-y-6">
        <div>
            <h1 class="page-title">New POS Order</h1>
            <p class="page-description">Create a point of sale order</p>
        </div>
        <div class="card">
            <div class="card-content">
                <form method="POST" action="{{ route('pos.store') }}" class="space-y-4">
                    @csrf
                        ...
                        <div id="items-container" class="space-y-2">
                            <div class="item-row grid grid-cols-12 gap-2 items-end">
                                <div class="col-span-5 space-y-1">
                                    ...
                                    <input type="text" name="items[0][name]" class="input-field item-name" required>
                                </div>
                                <div class="col-span-2 space-y-1">
                                    <label class="label text-xs">Qty</label>
                                    <input type="number" name="items[0][quantity]" class="input-field item-qty" min="1" value="1" required>
                                </div>
                                <div class="col-span-2 space-y-1">
                                    <label class="label text-xs">Price</label>
                                    <input type="number" name="items[0][unit_price]" class="input-field item-price" step="0.01" min="0" required>
                                </div>
                                <div class="col-span-2 space-y-1">
                                    <label class="label text-xs">Total</label>
                                    <input type="text" class="input-field item-total bg-gray-50" readonly>
                                </div>
